...
from kl_regu_cotta import setup_kl_regu_cotta
...
def evaluate_kl_regu_cotta(description):
    """Evaluate KL Regularized CoTTA on CIFAR-10C dataset."""
    # Parse command line arguments for KL regularization parameters
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--cfg", dest="cfg_file", type=str, required=True,
                        help="Config file location")
    parser.add_argument("--tau", dest="tau", type=float, default=1.0,
                        help="Temperature parameter for soft weighting (higher = smoother weighting)")
    parser.add_argument("--eps", dest="eps", type=float, default=1e-8,
                        help="Numerical stability parameter for weighted loss")
    parser.add_argument("--disable_kl_regu", action="store_true",
                        help="Disable KL regularization (equivalent to baseline CoTTA)")
    parser.add_argument("opts", default=None, nargs=argparse.REMAINDER,
                        help="See conf.py for all options")
    
    args = parser.parse_args()
    
    # Set KL regularization parameters
    if args.disable_kl_regu:
        tau = 0.0  # Disable regularization
        eps = 1e-8
    else:
        tau = args.tau
        eps = args.eps
    ...
    # Load configuration
    # Ensure config parser sees only config-related args (avoid '--tau', '--eps' etc.)
    ...
    # Setup KL Regularized CoTTA model
    model = setup_kl_regu_cotta(
        base_model, 
        optimizer,
        tau=tau,
        eps=eps,
    ...
    # Log configuration
    if tau == 0.0:
        logger.info("Running baseline CoTTA (KL regularization disabled)")
    else:
        logger.info(f"Running KL-Regu CoTTA with tau={tau}, eps={eps}")
    ...
    if tau == 0.0:
        ...
        logger.info(f"KL-Regu CoTTA Results (tau={tau}, eps={eps}):")
...
def setup_optimizer(params):
    """Set up optimizer for KL Regularized CoTTA adaptation.

    KL Regularized CoTTA needs an optimizer for test-time entropy minimization.
    In principle, KL Regularized CoTTA could make use of any gradient optimizer.
...
if __name__ == '__main__':
    evaluate_kl_regu_cotta('KL Regularized CoTTA CIFAR-10-C evaluation.')
