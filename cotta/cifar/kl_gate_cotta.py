...
class KLGateCoTTA(nn.Module):
    """KL-Gate CoTTA adapts a model by entropy minimization during testing.
    
    Adds a KL divergence gate to skip updates when teacher-student disagreement is high.
    """
...
def setup_kl_gate_cotta(model, optimizer, kl_threshold=0.1, steps=1, episodic=False, 
                       mt_alpha=0.999, rst_m=0.01, ap=0.92):
    """Set up KL-Gate CoTTA adaptation.

    Configure the model for training + feature modulation by batch statistics,
    collect the parameters for feature modulation by gradient optimization,
    set up the optimizer, and then wrap with KL-Gate CoTTA.
    """
    model = configure_model(model)
    params, param_names = collect_params(model)
    kl_gate_cotta_model = KLGateCoTTA(
        model, optimizer,
        steps=steps,
        episodic=episodic, 
        mt_alpha=mt_alpha, 
