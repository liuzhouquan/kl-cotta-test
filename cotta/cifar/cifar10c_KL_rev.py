...
from kl_gate_cotta_rev import setup_kl_gate_cotta_rev
...
def evaluate_kl_gate_cotta_rev(description):
    """Evaluate KL-Gate CoTTA Rev on CIFAR-10C dataset."""
    ...
    parser.add_argument("--thr", dest="kl_threshold", type=float, default=0.1,
                        help="KL divergence threshold (0.0 = baseline CoTTA, >0.0 = KL-Gate-Rev enabled)")
    parser.add_argument("--disable_kl_gate", action="store_true",
                        help="Disable KL-Gate-Rev (equivalent to thr=0.0)")
    ...
    # Setup KL-Gate CoTTA Rev model
    model = setup_kl_gate_cotta_rev(
        base_model, 
        optimizer,
    ...
    # Log configuration
    if kl_threshold == 0.0:
        logger.info("Running baseline CoTTA (KL-Gate-Rev disabled)")
    else:
        logger.info(f"Running KL-Gate-Rev CoTTA with threshold: {kl_threshold}")
    ...
    if kl_threshold == 0.0:
        ...
        logger.info(f"KL-Gate-Rev CoTTA Results (threshold={kl_threshold}):")
...
def setup_optimizer(params):
    """Set up optimizer for KL-Gate CoTTA Rev adaptation.

    KL-Gate CoTTA Rev needs an optimizer for test-time entropy minimization.
    In principle, KL-Gate CoTTA Rev could make use of any gradient optimizer.
...
if __name__ == '__main__':
    evaluate_kl_gate_cotta_rev('KL-Gate-Rev CoTTA CIFAR-10-C evaluation.')
