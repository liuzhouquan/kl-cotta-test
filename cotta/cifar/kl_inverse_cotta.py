...
class KLInverseCoTTA(nn.Module):
    """KL Inverse CoTTA adapts a model by inverse entropy weighting during testing.
    
    Uses inverse weighting based on KL divergence - difficult samples (high KL) 
    get higher weights, easy samples (low KL) get lower weights.
    """
    def __init__(self, model, optimizer, steps=1, episodic=False, mt_alpha=0.99, rst_m=0.1, ap=0.9, tau=1.0, eps=1e-8, strategy='exp'):
        super().__init__()
        self.model = model
        ...
        # KL Inverse parameters
        self.tau = tau  # Temperature parameter for inverse weighting
        self.eps = eps  # Numerical stability parameter
        self.strategy = strategy  # Weighting strategy: 'exp', 'reciprocal', 'linear'
    ...
    def compute_inverse_weights(self, kl_div):
        """Compute inverse weights based on KL divergence."""
        if self.strategy == 'exp':
            # Exponential inverse weighting: weights = exp(kl_div / tau)
            weights = torch.exp(kl_div / self.tau)
        elif self.strategy == 'reciprocal':
            # Reciprocal weighting: weights = 1 / (kl_div + eps)
            weights = 1.0 / (kl_div + self.eps)
        elif self.strategy == 'linear':
            # Linear inverse weighting: weights = 1 + kl_div / tau
            weights = 1.0 + kl_div / self.tau
        else:
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        # Compute inverse weights based on KL divergence
        weights = self.compute_inverse_weights(kl_div)
        ...
        # Log inverse weighting information
        logger.info(f"KL-Inverse: avg_KL={kl_div.mean().item():.4f}, "
...
def setup_kl_inverse_cotta(model, optimizer, tau=1.0, eps=1e-8, strategy='exp', steps=1, episodic=False, 
                          mt_alpha=0.999, rst_m=0.01, ap=0.92):
    """Set up KL Inverse CoTTA adaptation.

    Configure the model for training + feature modulation by batch statistics,
    collect the parameters for feature modulation by gradient optimization,
    set up the optimizer, and then wrap with KL Inverse CoTTA.
    """
    model = configure_model(model)
    params, param_names = collect_params(model)
    kl_inverse_cotta_model = KLInverseCoTTA(
        model, optimizer,
        steps=steps,
        episodic=episodic, 
        mt_alpha=mt_alpha, 
    ...
    logger.info(f"KL inverse tau: %s", tau)
    logger.info(f"KL inverse eps: %s", eps)
    logger.info(f"KL inverse strategy: %s", strategy)
    return kl_inverse_cotta_model
